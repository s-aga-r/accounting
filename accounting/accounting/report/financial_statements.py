...
def set_gl_entries_by_account(gl_entries_by_account, from_date, to_date, root_lft, root_rgt):
    ...
    sql_statement = """select posting_date, account, debit_amount, credit_amount from `tabGeneral Ledger`
        ...
        for entry in entries:
            ...
            if entry.posting_date < getdate(to_date):
                d["opening_balance"] = d.get(
                    "opening_balance", 0.0) + flt(entry.debit_amount) - flt(entry.credit_amount)
